...
SENDER_PASSWORD = "your-app-password"  # Use app-specific password, not your actual password

# Token expiration times (in seconds)
EMAIL_VERIFICATION_TOKEN_EXPIRY = 24 * 60 * 60  # 24 hours
PASSWORD_RESET_TOKEN_EXPIRY = 1 * 60 * 60  # 1 hour
...
def verify_token(token: str, token_type: str) -> Optional[str]:
    """Verify a token and return email if valid, None otherwise"""
    _ensure_token_store()
    tokens = json.loads(TOKEN_STORE_FILE.read_text())
...
def reset_password(email: str, new_password: str) -> Tuple[bool, str]:
    """Reset password for a user (admin or via token)"""
    users = load_users()
...
def log_audit_event(event_type: str, email: str, details: str = ""):
    """Log an audit event"""
    log_message = f"[{event_type}] User: {email}"
    if details:
